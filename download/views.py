...
def history_download(request, username, submit_time):
    year, month, day = list(map(int, submit_time.split("-")[: 3]))
    hour, minute, second = list(map(int, submit_time.split("-")[-1].split(":")))
    ...
    result_file = [os.path.join(upload_dir, f) for f in os.listdir(upload_dir) if f.endswith("json")][0]
    with open(result_file, "r") as f:
        data = json.load(f)
